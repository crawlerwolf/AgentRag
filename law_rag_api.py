...
from law_rag import LawRagAgent
...
# ---------- 全局 agent 实例 ----------
law_agent: Optional[LawRagAgent] = None
...
async def health_check():
    ...
    retriever_ready = law_agent.ensemble_retriever is not None if law_agent else False
...
async def chat(request: ChatRequest):
    ...
    """
    check_agent()
    if law_agent.ensemble_retriever is None:
    ...
    try:
        ...
        hit, answer = await law_agent.search_history(request.query, config)
        if hit:
            logger.info("历史缓存命中")
            return ChatResponse(answer=answer, thread_id=thread_id, cached=True)
        # 未命中，走完整 RAG
        final_msg = await law_agent.get_agent_message(request.query, config)
        await law_agent.update_history(request.query, final_msg.content)
        logger.info(f"回答生成完成 | thread_id={thread_id}")
        return ChatResponse(answer=final_msg.content, thread_id=thread_id, cached=False)
